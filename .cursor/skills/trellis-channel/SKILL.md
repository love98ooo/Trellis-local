...
`trellis-local channel` is the local multi-agent collaboration runtime. Reach for it when agents need to talk through a durable event log, when a worker should be spawned as a peer process, when an in-flight worker needs interrupt / debugging, or when feedback should be recorded on a durable `--type forum` channel.
...
```bash
trellis-local --version
trellis-local channel --help
trellis-local channel list --all
trellis-local channel list --scope global --all
```
...
trellis-local channel thread <board> <thread> --scope global
...
- Use `--context-file` / `--context-raw` and `trellis-local channel context add/delete/list`. `--linked-context-*` is deprecated terminology.
...
- Use additional research or review rounds only while concrete unresolved questions remain. Follow `.trellis/workflow.md` for authorization.
...
- Long-term memory retrieval. Use durable forum channels for actionable issues, and `trellis-local mem` (the `trellis-session-insight` skill) for session/history search.
