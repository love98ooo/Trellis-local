...
| `trellis-session-insight` | Wraps the `trellis-local mem` CLI so an AI knows when and how to reach into past Claude Code / Codex / Pi Agent conversation logs. |
...
| `trellis-channel` | Capability skill teaching an AI when to reach for `trellis-local channel` for multi-agent collaboration, forum/thread persistent boards, and dispatcher-wait patterns. |
...
1. `trellis-local init` → `configurePlatform(platformId, cwd)` → `writeTemplateMap(cwd, collect<Platform>Templates())`. For 18 of the 21 platforms the registry entry in `configurators/index.ts` is literally `fromTemplates(collect<Platform>Templates)`, which *is* that composition. Claude Code, Codex and ZCode spell out a `configure` of their own, each for work a `Map<path, content>` cannot express (an opt-in `--with-statusline` flag, an intentionally empty `.codex/skills/` directory, a one-shot console notice) — none of them restates the file list.
2. `trellis-local update` → `collectPlatformTemplates(platformId)` (in `configurators/index.ts`) → the same map, used to detect drift and to populate `.trellis/.template-hashes.json`.
...
4. **No dispatch wiring is required.** `listDirectories("bundled-skills")` discovers the new directory automatically, so all platforms receive it on the next `trellis-local init` or `trellis-local update`.
...
5. **Verify the distribution path** before shipping. Skipping any of these steps has historically caused features to be documented as bundled while the published npm tarball was missing the files:
   ...
   - In a fresh temp project, `trellis-local init` writes `.claude/skills/<skill>/SKILL.md`, `.agents/skills/<skill>/SKILL.md`, `.zcode/skills/<skill>/SKILL.md`, etc.
   - `.trellis/.template-hashes.json` lists the generated files.
   - `trellis-local update --dry-run` in that temp project reports "Already up to date!".

6. **Add a migration manifest entry** if the skill is added in a release that other projects will upgrade into. Without an explicit manifest entry the file will land via the standard "missing file" branch of `trellis-local update`, but a manifest makes the change visible in the changelog.
...
The supported pattern relies on the existing template-hash diff in `trellis-local update`:
...
3. The next `trellis-local update` detects the user modification and leaves the file untouched (Trellis never overwrites user-modified files without an explicit `--force`).
...
- A future `trellis-local update --force` will overwrite local edits. Keep the override under version control so it can be reapplied if needed.
...
1. **Delete the directory in each platform skill root.** `trellis-local update` will see the file missing, compare against `.template-hashes.json`, and treat the deletion the same as any other user modification — it will not silently re-create the directory unless `--force` is passed.
...
- Do not couple bundled skills to a specific CLI binary (e.g. `trellis-local mem`) without surfacing the dependency in the skill's description and references — users on older releases may not have the command.
