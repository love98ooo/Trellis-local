...
- Raw platform conversation logs queryable via `trellis-local mem`: `~/.claude/projects/`, `~/.codex/sessions/`, and `~/.pi/agent/sessions/` (OpenCode adapter degraded for the v0.6 line).
...
- `references/local-architecture/generated-files.md`: Files generated by `trellis-local init` and their customization boundaries, including `.trellis/agents/`.
...
- `references/local-architecture/spec-system.md`: How `.trellis/spec/` is organized and loaded as local project guidance.
- `references/local-architecture/workspace-memory.md`: `.trellis/workspace/` journals plus `trellis-local mem` cross-session recall and the `@mindfoldhq/trellis-core/mem` SDK.
- `references/local-architecture/context-injection.md`: Hooks, sub-agent preludes, and channel-runtime worker inbox routing.
- `references/local-architecture/multi-agent-channel.md`: `trellis-local channel` subcommands, project-scoped event store, forum/thread channels, worker OOM guard, durable idempotency, and bundled `.trellis/agents/` runtime agents.
...
- `references/customize-local/change-workflow.md`: Change phases, routing, next actions, workflow-state, and workflow context loading.
...
- `references/customize-local/change-context-loading.md`: Change how tasks, specs, journals, hook context, channel inbox messages, and `trellis-local mem` recall are loaded.
...
- `references/customize-local/change-spec-structure.md`: Adjust the project spec structure under `.trellis/spec/`, without automatic registry refresh.
- `references/customize-local/add-project-local-conventions.md`: Put personal project rules into project-local specs or local skills.

## Current Rules

- `.trellis/workflow.md` is the single local workflow source, generated from the fork CLI template.
- `.trellis/config.yaml` configures local hooks, packages, journal line limits, channel worker safeguards and Codex dispatch.
- `.trellis/spec/` stores local project conventions; ordinary update does not refresh them from a registry.
...
- `.trellis/workspace/` stores **deliberately written** personal journals. Raw cross-session dialogue is **not** stored here — it lives on disk under `~/.claude/projects/`, `~/.codex/sessions/`, and `~/.pi/agent/sessions/` and is recovered via `trellis-local mem search|extract|context`. The bundled `trellis-session-insight` skill teaches when to reach for `mem`.
- `.trellis/agents/{check,implement}.md` are bundled, platform-agnostic channel runtime agent definitions loaded by `trellis-local channel spawn --agent <name>`. Editable; `trellis-local update` backfills missing ones. Editing the per-platform `trellis-implement.md` / `trellis-check.md` does **not** change channel-runtime worker behavior.
...
- Bundled multi-file skills (`trellis-meta`, `trellis-spec-bootstrap`, `trellis-session-insight`, `trellis-channel`) are auto-dispatched to every platform skill root by `getBundledSkillTemplates()` in `packages/cli/src/templates/common/index.ts`. Dropping a new directory under `packages/cli/src/templates/common/bundled-skills/` (upstream) ships it to every platform on the next `trellis-local update`.
...
- Do not put project-private project rules into any public bundled skill (`trellis-meta`, `trellis-spec-bootstrap`, `trellis-session-insight`, `trellis-channel`); put project rules in `.trellis/spec/`, a project-local skill, the current task, or the workspace journal — `trellis-local update` will overwrite anything inside a bundled skill directory.
- Do not hand-edit `~/.trellis/channels/<project>/<channel>/events.jsonl`; sequence numbers are assigned under a file lock and replay-safe writes go through the `trellis-local channel` CLI or the `@mindfoldhq/trellis-core/channel` SDK.
...
- Do not describe removed or never-shipped mechanisms as current Trellis behavior; cross-check against the local `.trellis/config.yaml` and the installed CLI's `trellis-local --help` before claiming a knob exists.
