...
```bash
{{PYTHON_CMD}} ./.trellis/scripts/task.py complete <completed-task> --reason "Acceptance evidence"
{{PYTHON_CMD}} ./.trellis/scripts/task.py archive <completed-task>
```

Record progress, checks, remaining work and existing business-code commit references when available:

```bash
{{PYTHON_CMD}} ./.trellis/scripts/add_session.py --title "Session Title" --summary "Progress and checks"
```
