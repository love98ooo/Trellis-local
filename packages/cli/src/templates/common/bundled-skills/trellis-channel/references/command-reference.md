...
Authoritative current command reference for `trellis-local channel` subcommands,
...
```
trellis-local channel <subcommand>
```
...
```bash
trellis-local channel create <name>
...
```bash
trellis-local channel send <name> [text]
...
```bash
trellis-local channel messages <name>
  [--scope project|global]
...
```bash
trellis-local channel wait <name>
...
- Use `trellis-local channel interrupt` (the command) only when you actually want
...
```bash
trellis-local channel send T --as A --stdin < /tmp/message.md
trellis-local channel send T --as A --text-file /tmp/message.md
```
...
```bash
trellis-local channel interrupt <name> [text]
...
```bash
trellis-local channel spawn <name>
  [--scope project|global]
...
```bash
trellis-local channel run [name?]
  [--agent <name>]
...
```bash
trellis-local channel kill <name>
...
```bash
trellis-local channel post <name> <action>
  --as <agent>                            # REQUIRED
  [--scope project|global]
...
- `--labels` are replace-semantics, not append.
- Output: appended event JSON on stdout.

### `forum <name>`

```bash
trellis-local channel forum <name>
  [--scope project|global]
  [--status <status>]
  [--raw]
```
...
```bash
trellis-local channel thread <name> <thread-key>
  [--scope project|global]
  [--raw]

trellis-local channel thread rename <name> <old-thread> <new-thread>
  --as <agent>                            # REQUIRED
  [--scope project|global]
```
...
```bash
trellis-local channel context add <name>
...
trellis-local channel context delete <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
  [--thread <key>]
  [--file <abs-path>] ...
...
trellis-local channel context list <name>
  [--scope project|global]
  [--thread <key>]
...
```bash
trellis-local channel title set <name>
  --title <text>                          # REQUIRED
  [--as <agent>]                          # default: main
  [--scope project|global]

trellis-local channel title clear <name>
  [--as <agent>]                          # default: main
  [--scope project|global]
```
