 ...
 * before that fix can still hold one, and `trellis-local update` does not rewrite
...
function buildContext(root: string, agent: string, key: string | null): string {
  const dir = readTaskDir(root, key);
  if (!dir)
    return "No active Trellis task. Follow .trellis/workflow.md and proceed with the authorized request.";
...
export default function trellisExtension(pi: {
  ...
  pi.on?.("session_start", (event, ctx) => {
    getKey(event, ctx);
    ctx?.ui?.notify?.(
      "Trellis project context is available. Use /trellis-start for session context or /trellis-continue to resume.",
      "info",
    );
  });
